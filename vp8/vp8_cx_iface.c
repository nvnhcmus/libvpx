...
static __inline vpx_codec_err_t validate_img(vpx_codec_alg_priv_t *ctx,
                                       ...
                                       vpx_codec_priv_enc_mr_cfg_t *mr_cfg)
{
    ...
    {
        memcpy (oxcf->target_bitrate,	cfg.ts_target_bitrate,	sizeof(cfg.ts_target_bitrate));
        memcpy (oxcf->rate_decimator,	cfg.ts_rate_decimator,	sizeof(cfg.ts_rate_decimator));
...
static INLINE void pick_quickcompress_mode(vpx_codec_alg_priv_t  *ctx,
    ...
    if (!res && ctx->cpi)
    {
        ...
        while (cx_data_sz >= ctx->cx_data_sz / 2)
        {
            ...
            if (size)
            {
                ...
                {
                    int i;
                    const int num_partitions = (1 << cpi->common.multi_token_partition) + 1;
